...
- 首次执行 `pnpm compiler release package --source-hash 08496ec8927e15f59365319b243556f69922b42c6c7aa9635bde79a005d09e5e --previous-release rel-dc997f599668b817` 时被 `RELEASE_GATE_UNMET` 拒绝：`IMAGE_EXTRACT` 账本状态为 `FAILED`（2026-09-18 的 `MEDIA_CONFIG_INVALID`）。后续结构、卡片、音频账本仍是内容修订前的记录，不能将旧产物重新命名为新发布。
- 已用原始 PDF 路径重新运行到 `IMAGE_EXTRACT`，本地账本转为 `PASSED`（440 页、约 28 秒）；再次打包到下一道门禁时被 `RELEASE_SOURCE_CONTENT_INVALID` 拒绝：旧 `normalized.jsonl` 有 6,940 条来源归属异常。它是修订前编译产物，不是当前独立汇总审计的 0 条；需要在新 OCR 修订基础上重跑结构归一化及其下游语义/卡片/音频阶段，再校对来源内容。不得把旧 `normalized.jsonl` 当作新内容发布。
